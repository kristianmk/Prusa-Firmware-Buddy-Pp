...
WizardState_t StateFnc_FIRSTLAY_MSBX_REPEAT_PRINT() {
    if (marlin_error(MARLIN_ERR_ProbingFailed)) {
        ...
        static const char en_text_probing[] = N_("Mesh bed leveling failed. Make sure there is a sheet on the bed. Do you want to repeat it or cancel?");
        if (MsgBox(_(en_text_probing), Responses_YesAbort, 1) == Response::Yes) {
